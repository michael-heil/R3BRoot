...
// First see if a given FiberI is present (there is probably a better way to do it):
      ...
      for(Int_t ifibcount = 0; ifibcount < 14; ifibcount++){
        ...
        std::stringstream histName1,histName2,histName3,histName4,histName5,histName6,histName7,histName8,histName9,histName10;
        std::stringstream histTitle1,histTitle2,histTitle3,histTitle4,histTitle5,histTitle6,histTitle7,histTitle8,histTitle9,histTitle10;	   	    	 
	...
	// get access to cal data     
      ...
      // if(ifibdet < 3 || ifibdet > 4) tempFibName<<ifibdet;
	  // if(ifibdet == 3) tempFibName<<ifibdet<<"a";
	   // if(ifibdet == 4) tempFibName<<ifibcount<<"b";	    
	   tempFibNames = FiName.str(); //tempFibName.str();
       cfibName = tempFibNames.c_str();
      // tempCanvName<<"Fiber"<<tempFibNames;
       stempCanvNames = FiName.str(); //tempCanvName.str();
               		  
       cout<<"I am creating canvas "<<stempCanvNames.c_str()<<endl;
               		  
               		  
	   FibCanvas[ifibcount]=new TCanvas(stempCanvNames.c_str(),stempCanvNames.c_str(), 10, 10, 810, 710);
...
//TofD detector
	...
	if(fMappedItemsTofd){
		...
		for (Int_t j = 0; j < 4; j++)
		{
		     ...
		     fh_tofd_ToF[j] = new TH2F(strName7, strName8, 15,0,15,40000, 1900., 2000.);
